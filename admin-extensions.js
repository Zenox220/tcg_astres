...
(() => {
  ...
  const PACKS = [
    ...
    ["basic", "Paquet Basic"], ["premium", "Paquet Premium"], ["elite", "Paquet Élite"]
  ];
  const HEROES = [
  ...
  function shopCardOptions(selected = "") {
    return cardsCatalog().filter(card => card.collectible !== false && card.type !== "token").map(card => `<option value="${esc(card.id)}" ${card.id === selected ? "selected" : ""}>${esc(card.name || card.id)} · ${esc(card.id)}</option>`).join("");
  }
  ...
  function heroOptions(selected = "") {
    return HEROES.map(([id, name]) => `<option value="${id}" ${id === selected ? "selected" : ""}>${name}</option>`).join("");
  }
  function rewardLabel(rewards = []) {
    return rewards.map(reward => {
      ...
      if (reward.type === "hero") return `Héros ${HEROES.find(([id]) => id === reward.heroId)?.[1] || reward.heroId}`;
  ...
  function addRewardRow(containerId, reward = { type: "coins", amount: 100 }) {
    ...
    row.innerHTML = `
      <label><span>Type</span><select data-reward-type><option value="coins">Pièces</option><option value="pack">Paquet</option><option value="card">Carte</option><option value="hero">Héros</option></select></label>
    ...
    const update = () => {
      ...
      if (type.value === "hero") id.innerHTML = heroOptions(reward.heroId || id.value || "pretresse_lunaire");
      row.querySelector("[data-reward-amount]").closest("label")?.classList.toggle("hidden", type.value === "hero");
    };
  ...
  function readRewards(containerId) {
    return [...($(containerId)?.querySelectorAll(".admin-reward-row") || [])].map(row => {
      ...
      if (type === "card") return { type, amount, cardId: id };
      return { type: "hero", amount: 1, heroId: id };
    });
  }
  ...
  function addEventQuestRow(quest = {}) {
    ...
    row.innerHTML = `
      ...
      <label><span>Récompense</span><select data-event-quest-reward-type><option value="coins">Pièces</option><option value="pack">Paquet</option><option value="card">Carte</option><option value="hero">Héros</option></select></label>
    ...
    const update = () => {
      ...
      if (rewardType.value === "hero") select.innerHTML = heroOptions(quest.reward?.heroId || select.value || "pretresse_lunaire");
      row.querySelector("[data-event-quest-reward-amount]").closest("label")?.classList.toggle("hidden", rewardType.value === "hero");
    };
  ...
  function readEventQuests() {
    return [...($("admin-event-quests")?.querySelectorAll(".admin-event-quest-row") || [])].map((row, index) => {
      ...
      const reward = type === "coins" ? { type, amount } : type === "pack" ? { type, amount, packId: id } : type === "card" ? { type, amount, cardId: id } : { type: "hero", amount: 1, heroId: id };
      return {
  ...
  function populateCatalogs() {
    ...
    const cardShopSelect = $("admin-card-shop-card");
    if (cardShopSelect) cardShopSelect.innerHTML = shopCardOptions();
    const heroSelect = $("admin-hero-id");
    if (heroSelect) heroSelect.innerHTML = heroOptions();
  }
  ...
  function setDefaultDates() {
    ...
    const defaults = {
      ...
      "admin-card-shop-start": now, "admin-card-shop-end": now + 7 * 86400000,
  ...
  function renderPlayerHeroes() {
    const summary = $("admin-player-heroes-summary");
    if (!summary) return;
    const unlocked = new Set(Array.isArray(state.currentUser?.progress?.unlockedHeroes) ? state.currentUser.progress.unlockedHeroes : ["reine_celeste"]);
    unlocked.add("reine_celeste");
    summary.textContent = `Débloqués : ${HEROES.filter(([id]) => unlocked.has(id)).map(([, name]) => name).join(", ")}.`;
  }
  ...
  function bindSanctions() {
    document.addEventListener("astrea:admin-user-loaded", event => {
      ...
      if (state.currentUser) { loadModeration(state.currentUser.id); renderPlayerHeroes(); }
    });
    $("admin-hero-form")?.addEventListener("submit", async event => {
      event.preventDefault();
      if (!state.currentUser) return toast("Sélectionnez d’abord un joueur.", true);
      const heroId = $("admin-hero-id").value;
      const unlocked = $("admin-hero-unlocked").value === "true";
      try {
        const data = await api(`/api/admin/users/${state.currentUser.id}/heroes`, { method: "POST", body: { heroId, unlocked } });
        state.currentUser.progress = data.progress || state.currentUser.progress;
        renderPlayerHeroes();
        toast(unlocked ? "Héros débloqué pour le joueur." : "Héros verrouillé pour le joueur.");
  ...
  // ---------------- Boutique ----------------
  function bindShop() {
    $("admin-offer-form")?.addEventListener("submit", saveOffer);
    $("admin-offer-cancel")?.addEventListener("click", resetOfferForm);
    $("admin-offers-refresh")?.addEventListener("click", loadOffers);
    $("admin-shop-packs-refresh")?.addEventListener("click", loadPackSettings);
    $("admin-shop-heroes-refresh")?.addEventListener("click", loadHeroSettings);
    $("admin-card-shop-form")?.addEventListener("submit", saveCardListing);
    $("admin-card-shop-cancel")?.addEventListener("click", resetCardListingForm);
    $("admin-card-shop-refresh")?.addEventListener("click", loadCardListings);
  }

  async function loadShop() {
    await Promise.all([loadPackSettings(), loadHeroSettings(), loadCardListings(), loadOffers()]);
  }

  async function loadPackSettings() {
    try {
      const data = await api("/api/admin/shop-packs");
      state.packSettings = data.packs || [];
      renderPackSettings();
    } catch (error) { toast(error.message, true); }
  }

  function renderPackSettings() {
    const root = $("admin-shop-packs-list");
    if (!root) return;
    if (!state.packSettings.length) {
      root.innerHTML = '<div class="admin-empty-state compact"><p>Aucun paquet configuré.</p></div>';
      return;
    }
    root.innerHTML = state.packSettings.map(item => `
      <article class="admin-live-item admin-pack-shop-item">
        <div>
          <div class="admin-popup-list-title-row"><strong>${esc(item.name || item.packId)}</strong>${item.enabled ? '<span class="admin-popup-status active">Achat activé</span>' : '<span class="admin-popup-status disabled">Achat désactivé</span>'}</div>
          <p>Identifiant : ${esc(item.packId)}</p>
          <small>Prix d’origine : ${Number(item.defaultPrice || 0).toLocaleString("fr-FR")} pièces</small>
        </div>
        <div class="admin-pack-shop-controls">
          <label><span>Prix</span><input data-pack-price="${esc(item.packId)}" type="number" min="0" value="${Number(item.price || 0)}" /></label>
          <label class="admin-popup-checkbox-row"><input data-pack-enabled="${esc(item.packId)}" type="checkbox" ${item.enabled ? "checked" : ""} /><span>Achat actif</span></label>
          <button class="admin-btn admin-btn-primary" data-save-pack="${esc(item.packId)}" type="button">Enregistrer</button>
        </div>
      </article>`).join("");
    root.querySelectorAll("[data-save-pack]").forEach(button => button.addEventListener("click", async () => {
      const packId = button.dataset.savePack;
      const price = Number(root.querySelector(`[data-pack-price="${packId}"]`)?.value || 0);
      const enabled = Boolean(root.querySelector(`[data-pack-enabled="${packId}"]`)?.checked);
      try {
        await api(`/api/admin/shop-packs/${encodeURIComponent(packId)}`, { method: "PUT", body: { price, enabled } });
        toast(`${packId} mis à jour.`);
  ...
  async function loadHeroSettings() {
    try {
      const data = await api("/api/admin/shop-heroes");
      state.heroSettings = data.heroes || [];
      renderHeroSettings();
    } catch (error) { toast(error.message, true); }
  }

  function renderHeroSettings() {
    const root = $("admin-shop-heroes-list");
    if (!root) return;
    if (!state.heroSettings.length) { root.innerHTML = '<div class="admin-empty-state compact"><p>Aucun héros configuré.</p></div>'; return; }
    root.innerHTML = state.heroSettings.map(item => `
      <article class="admin-live-item admin-pack-shop-item">
        <div><div class="admin-popup-list-title-row"><strong>${esc(item.name || item.heroId)}</strong>${item.heroId === "reine_celeste" ? '<span class="admin-popup-status scheduled">Héros de départ</span>' : item.enabled ? '<span class="admin-popup-status active">Achat activé</span>' : '<span class="admin-popup-status disabled">Achat désactivé</span>'}</div>
        <p>Identifiant : ${esc(item.heroId)}</p><small>Prix conseillé : ${Number(item.defaultPrice || 0).toLocaleString("fr-FR")} pièces</small></div>
        <div class="admin-pack-shop-controls"><label><span>Prix</span><input data-hero-price="${esc(item.heroId)}" type="number" min="0" value="${Number(item.price || 0)}" ${item.heroId === "reine_celeste" ? "disabled" : ""}></label>
        <label class="admin-popup-checkbox-row"><input data-hero-enabled="${esc(item.heroId)}" type="checkbox" ${item.enabled ? "checked" : ""} ${item.heroId === "reine_celeste" ? "disabled" : ""}><span>Achat actif</span></label>
        <button class="admin-btn admin-btn-primary" data-save-hero="${esc(item.heroId)}" type="button" ${item.heroId === "reine_celeste" ? "disabled" : ""}>Enregistrer</button></div>
      </article>`).join("");
    root.querySelectorAll("[data-save-hero]").forEach(button => button.addEventListener("click", async () => {
      const heroId = button.dataset.saveHero;
      const price = Number(root.querySelector(`[data-hero-price="${heroId}"]`)?.value || 0);
      const enabled = Boolean(root.querySelector(`[data-hero-enabled="${heroId}"]`)?.checked);
      try { await api(`/api/admin/shop-heroes/${encodeURIComponent(heroId)}`, { method: "PUT", body: { price, enabled } }); toast(`${heroId} mis à jour.`); await loadHeroSettings(); }
      catch (error) { toast(error.message, true); }
    }));
  }

  async function loadCardListings() {
    try {
      const data = await api("/api/admin/card-shop");
      state.cardListings = data.listings || [];
      renderCardListings();
    } catch (error) { toast(error.message, true); }
  }

  async function saveCardListing(event) {
    event.preventDefault();
    const id = $("admin-card-shop-id").value;
    const body = {
      cardId: $("admin-card-shop-card").value,
      price: Number($("admin-card-shop-price").value),
      originalPrice: $("admin-card-shop-original-price").value === "" ? null : Number($("admin-card-shop-original-price").value),
      quantity: Number($("admin-card-shop-quantity").value || 1),
      purchaseLimit: Number($("admin-card-shop-limit").value || 0),
      startsAt: iso("admin-card-shop-start"),
      endsAt: iso("admin-card-shop-end"),
      enabled: $("admin-card-shop-enabled").checked
    };
    try {
      await api(id ? `/api/admin/card-shop/${id}` : "/api/admin/card-shop", { method: id ? "PUT" : "POST", body });
      toast(id ? "Carte en boutique modifiée." : "Carte ajoutée à la boutique.");
      resetCardListingForm();
      await loadCardListings();
    } catch (error) { toast(error.message, true); }
  }

  function editCardListing(id) {
    const item = state.cardListings.find(entry => entry.id === String(id));
    if (!item) return;
    $("admin-card-shop-id").value = item.id;
    $("admin-card-shop-card").value = item.cardId;
    $("admin-card-shop-price").value = item.price;
    $("admin-card-shop-original-price").value = item.originalPrice ?? "";
    $("admin-card-shop-quantity").value = item.quantity || 1;
    $("admin-card-shop-limit").value = item.purchaseLimit || 0;
    $("admin-card-shop-start").value = toInput(item.startsAt);
    $("admin-card-shop-end").value = toInput(item.endsAt);
    $("admin-card-shop-enabled").checked = item.enabled !== false;
    $("admin-card-shop-form-title").textContent = "Modifier la carte en boutique";
    $("admin-card-shop-cancel").classList.remove("hidden");
    $("admin-card-shop-form").scrollIntoView({ behavior: "smooth" });
  }

  function resetCardListingForm() {
    $("admin-card-shop-form").reset();
    $("admin-card-shop-id").value = "";
    $("admin-card-shop-form-title").textContent = "Ajouter une carte en boutique";
    $("admin-card-shop-cancel").classList.add("hidden");
    $("admin-card-shop-price").value = 500;
    $("admin-card-shop-quantity").value = 1;
    $("admin-card-shop-limit").value = 1;
    $("admin-card-shop-start").value = toInput(Date.now());
    $("admin-card-shop-end").value = toInput(Date.now() + 7 * 86400000);
    $("admin-card-shop-enabled").checked = true;
  }

  function renderCardListings() {
    const root = $("admin-card-shop-list");
    if (!root) return;
    if (!state.cardListings.length) {
      root.innerHTML = '<div class="admin-empty-state compact"><p>Aucune carte programmée.</p></div>';
      return;
    }
    root.innerHTML = state.cardListings.map(item => {
      const card = cardsCatalog().find(entry => entry.id === item.cardId);
      return `<article class="admin-live-item">
        <div>
          <div class="admin-popup-list-title-row"><strong>${esc(card?.name || item.cardId)}</strong>${statusBadge(item.enabled, item.startsAt, item.endsAt)}</div>
          <p>${esc(card?.description || item.cardId)}</p>
          <small>${item.originalPrice && item.originalPrice > item.price ? `<s>${Number(item.originalPrice).toLocaleString("fr-FR")}</s> ` : ""}${Number(item.price).toLocaleString("fr-FR")} pièces · ${item.quantity || 1} copie(s) · limite ${item.purchaseLimit || "∞"} · fin ${dateText(item.endsAt)}</small>
        </div>
        <div class="admin-popup-list-actions"><button class="admin-btn admin-btn-secondary" data-edit-card-shop="${item.id}" type="button">Modifier</button><button class="admin-btn admin-btn-danger" data-delete-card-shop="${item.id}" type="button">Supprimer</button></div>
      </article>`;
    }).join("");
    root.querySelectorAll("[data-edit-card-shop]").forEach(button => button.addEventListener("click", () => editCardListing(button.dataset.editCardShop)));
    root.querySelectorAll("[data-delete-card-shop]").forEach(button => button.addEventListener("click", async () => {
      if (!confirm("Supprimer cette carte de la boutique ?")) return;
      try {
        await api(`/api/admin/card-shop/${button.dataset.deleteCardShop}`, { method: "DELETE" });
        toast("Carte retirée de la boutique.");
        await loadCardListings();
      } catch (error) { toast(error.message, true); }
    }));
  }
  ...
  const loaders = { cards: loadCards, events: async()=>{await loadEvents();}, quests: async()=>{if(!state.events.length)await loadEvents();await loadQuests();}, shop: loadShop, maintenance: loadMaintenance, codes: loadCodes, logs: loadLogs };
