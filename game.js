...
function loadProgress() {
  let loaded = null;
  try { loaded = JSON.parse(storageGet(STORAGE_KEY)); } catch (_) { loaded = null; }

  const collection = {};
  COLLECTIBLE_CARDS.forEach(card => {
    const loadedCount = Number(loaded?.collection?.[card.id] || 0);
    collection[card.id] = Math.max(STARTER_COLLECTION_COUNTS[card.id] || 0, loadedCount);
  });
  ...
  storageSet(STORAGE_KEY, JSON.stringify(result));
  return result;
}
...
function renderCollection() {
  ...
  cards.forEach(card => {
    ...
    el.type = "button";
    el.className = `collection-card rarity-${card.rarity}${canAdd ? " can-add" : ""}${owned <= 0 ? " unowned" : ""}`;
    el.disabled = !canAdd;
    el.innerHTML = `
      ...
      <span class="collection-owned">Possédées ${owned} · Deck ${inDeck}/${maxCopies}</span>`;
...
function addCardToDeck(cardId) {
  ...
  if (!card || progress.deck.length >= CONFIG.DECK_SIZE) return;
...
function createPlayerState(id, heroDefinition) {
  ...
  return {
    ...
    nextSpellDiscount:0, location:null, cardsPlayedThisTurn:0, spellsPlayedThisTurn:0, cardsDrawnTotal:0
  };
}
...
function createMinionInstance(card,ownerId,overrides={}) {
  ...
  return {
    ...
    dormantTurns,reincarnationUsed:Boolean(overrides.reincarnationUsed),frozen:Boolean(overrides.frozen),temporaryControl:overrides.temporaryControl||null
  };
}
...
function beginTurn(playerId) {
  ...
  p.cardsPlayedThisTurn=0;p.spellsPlayedThisTurn=0;
...
function chooseBestAiPlay() {
  ...
  gameState.ai.hand.forEach((instance,handIndex)=>{
    ...
    if(card.type==="creature")score=card.attack+(card.health*.75)+(card.keywords?.includes("taunt")?3:0)+(card.keywords?.includes("charge")?2:0);
...
async function playCardFromHand(playerId,handIndex,target=null) {
  ...
  if(card.type==="creature") {
    playedMinion=createMinionInstance(card,playerId);p.board.push(playedMinion);markTauntEntrance(playedMinion);window.ASTREA_CARD_AUDIO?.play(card);
...
function resolveEffect(effect,context={}) {
  ...
  switch(effect.effect) {
    ...
    case "buffStats": {const m=findMinionById(target?.instanceId);if(m)buffMinion(m,effect.attack||0,effect.health||0);break;}
...
function silenceMinion(m){m.keywords=[];m.effects=[];m.divineShieldActive=false;m.attack=m.baseAttack;m.maxHealth=m.baseHealth;m.currentHealth=Math.min(m.currentHealth,m.maxHealth);m.silenced=true;addLog(`${m.name} est réduit au silence.`);}
...
function canDropCreatureOnPlayerBoard(){if(!draggedPayload||draggedPayload.type!=="handCard"||!canPlayerInteract())return false;const inst=gameState.player.hand.find(c=>c.instanceId===draggedPayload.instanceId);return !!inst&&CARD_BY_ID[inst.cardId].type==="creature"&&isCardPlayable("player",inst);}

function isCardPlayable(playerId,instance){const p=gameState[playerId],card=CARD_BY_ID[instance.cardId];if(gameState.status!=="playing"||gameState.activePlayerId!==playerId)return false;if(p.mana.current<getCardCost(instance,playerId))return false;if(card.type==="creature"&&p.board.length>=CONFIG.MAX_BOARD)return false;if(card.type==="secret"&&(p.secrets.length>=CONFIG.MAX_SECRETS||p.secrets.some(s=>s.cardId===card.id)))return false;if(card.type==="quest"&&(p.quests.length>=3||p.quests.some(q=>q.cardId===card.id)))return false;if(cardNeedsTarget(card)&&!getValidTargetsForEffect(playerId,getManualEffect(card)).length)return false;return true;}
...
function isManualTargetType(type){return["friendlyCreature","enemyCreature","anyCreature","damagedCreature","friendlyCharacter","enemyCharacter","anyCharacter"].includes(type);}
function getValidTargetsForEffect(ownerId,effect){
  ...
  switch(effect.target){
    case"friendlyCreature":return fm;case"enemyCreature":return em;case"anyCreature":return[...fm,...em];
