...
(() => {
  ...
  function rewardText(rewards) {
    ...
    return list.map(item => {
      ...
      if (item.type === "hero") return `Héros ${HERO_DEFINITIONS?.[item.heroId]?.name || item.heroId || "inconnu"}`;
      return "";
  ...
  async function claimEventReward(eventId) {
    if (!token()) {
      toast("Connectez-vous pour récupérer la récompense de cet événement.", true);
      return;
    }
    try {
      await window.astreaFlushAccountProgress?.();
      const data = await api(`/api/events/${encodeURIComponent(eventId)}/claim`, { method: "POST" });
      applyServerProgress(data.progress);
  ...
  function renderHomeEvents() {
    const zone = document.getElementById("home-events-zone");
    if (!zone) return;
    ...
    zone.innerHTML = `<div class="home-events-heading">
      <div><p class="eyebrow">Live Astréa</p><h2>Événements en cours</h2></div>
      <span>${events.length} événement${events.length > 1 ? "s" : ""} actif${events.length > 1 ? "s" : ""}</span>
    </div>
    <div class="home-events-grid">${events.map(event => {
      ...
      const rewardParts = [];
      if ((event.rewards || []).length) rewardParts.push(rewardText(event.rewards));
      if (event.exclusiveCards?.length) rewardParts.push(`${event.exclusiveCards.length} carte(s) exclusive(s)`);
      const rewardSummary = rewardParts.join(" · ");
      return `<article class="home-event-card">
        <div class="home-event-visual${event.imageUrl ? "" : " no-image"}">
          ${event.imageUrl ? `<img src="${esc(event.imageUrl)}" alt="${esc(event.title)}">` : `<span class="home-event-sigil" aria-hidden="true">✦</span>`}
          <span class="home-event-live-badge">En cours</span>
        </div>
        <div class="home-event-content">
          <div class="home-event-title-row"><h3>${esc(event.title)}</h3><span>Jusqu’au ${fmtDate(event.endsAt)}</span></div>
          <p>${esc(event.description || "Un événement limité est actuellement disponible.")}</p>
          <div class="liveops-chip-row">
            <span class="liveops-chip">${event.quests?.length || 0} quête(s)</span>
            ${event.exclusiveCards?.length ? `<span class="liveops-chip">${event.exclusiveCards.length} carte(s) exclusive(s)</span>` : ""}
          </div>
          ${claimable ? `<strong class="home-event-reward">Récompense : ${esc(rewardSummary)}</strong>` : ""}
          <div class="home-event-actions">
            ${claimable ? `<button class="${claimed ? "secondary-btn" : "primary-btn"}" data-home-event-claim="${esc(event.id)}" type="button" ${claimed ? "disabled" : ""}>${claimed ? "Récompense récupérée" : token() ? "Récupérer la récompense" : "Se connecter pour récupérer"}</button>` : ""}
            ${event.quests?.length ? `<button class="secondary-btn" data-home-event-quests type="button">Voir les quêtes</button>` : ""}
          </div>
        </div>
      </article>`;
    }).join("")}</div>`;

    zone.querySelectorAll(".home-event-visual img").forEach(image => image.addEventListener("error", () => {
      const visual = image.closest(".home-event-visual");
      image.remove();
      visual?.classList.add("no-image");
      if (visual && !visual.querySelector(".home-event-sigil")) visual.insertAdjacentHTML("afterbegin", '<span class="home-event-sigil" aria-hidden="true">✦</span>');
    }, { once: true }));
    zone.querySelectorAll("[data-home-event-claim]").forEach(button => button.addEventListener("click", () => claimEventReward(button.dataset.homeEventClaim)));
    zone.querySelectorAll("[data-home-event-quests]").forEach(button => button.addEventListener("click", () => {
  ...
  function renderEvents() {
    ...
    zone.querySelectorAll("[data-event-claim]").forEach(button => button.addEventListener("click", () => claimEventReward(button.dataset.eventClaim)));
  }
  ...
  function installHooks() {
    ...
    if (typeof renderMenuSummary === "function" && !renderMenuSummary.__liveopsHomeEventsWrapped) {
      const original = renderMenuSummary;
      const wrapped = function() { const result = original.apply(this, arguments); renderHomeEvents(); return result; };
      wrapped.__liveopsHomeEventsWrapped = true;
      renderMenuSummary = wrapped;
    }
  ...
  window.astreaLiveAdmin = { receive, refresh: refreshConfig, toast, getConfig: () => state.config };
