/* Chroniques d'Astréa V39 — LiveOps pilotés depuis le panel administrateur */
(() => {
  "use strict";
  ...
  const state = { config: null, originalCards: new Map(), moderation: null, timer: null };
  ...
  function renderShopLiveOps() { renderEvents(); renderRemoteShop(); renderGiftCode(); }
  ...
  function installHooks() {
    ...
    if (typeof buyPack === "function" && !buyPack.__liveopsWrapped) {
      const original = buyPack;
      const wrapped = function(packId) {
        const beforeCoins = Number(progress?.coins || 0);
        const result = original.apply(this, arguments);
        const afterCoins = Number(progress?.coins || 0);
        if (afterCoins < beforeCoins) logClient("purchase", { type: "pack", packId, price: beforeCoins - afterCoins });
        return result;
      };
      wrapped.__liveopsWrapped = true;
      buyPack = wrapped;
