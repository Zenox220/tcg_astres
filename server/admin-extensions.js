...
const PACK_CATALOG = Object.freeze({
  astral: { id: "astral", name: "Pack Astral", defaultPrice: 300 },
  zenith: { id: "zenith", name: "Pack du Zénith", defaultPrice: 520 },
  neant: { id: "neant", name: "Pack du Néant", defaultPrice: 520 },
  quetes: { id: "quetes", name: "Pack des Quêtes", defaultPrice: 740 },
  basic: { id: "basic", name: "Paquet Basic", defaultPrice: 250 },
  premium: { id: "premium", name: "Paquet Premium", defaultPrice: 450 },
  elite: { id: "elite", name: "Paquet Élite", defaultPrice: 750 }
});

const DEFAULT_HERO_ID = "reine_celeste";
const HERO_CATALOG = Object.freeze({
  reine_celeste: { id: "reine_celeste", name: "Reine céleste", defaultPrice: 0, defaultUnlocked: true },
  pretresse_lunaire: { id: "pretresse_lunaire", name: "Prêtresse lunaire", defaultPrice: 900 },
  roi_solaire: { id: "roi_solaire", name: "Roi solaire", defaultPrice: 1100 },
  sorceleur_du_vide: { id: "sorceleur_du_vide", name: "Sorceleur du vide", defaultPrice: 1200 },
  architecte_marees: { id: "architecte_marees", name: "Architecte des Marées", defaultPrice: 1300 },
  chasseur_etoiles: { id: "chasseur_etoiles", name: "Chasseur d’Étoiles", defaultPrice: 1300 },
  grikz_bricoleur: { id: "grikz_bricoleur", name: "Grikz le Bricoleur", defaultPrice: 1400 },
  thorga_appel_esprits: { id: "thorga_appel_esprits", name: "Thorga l’Appel-esprits", defaultPrice: 1500 },
  nyx_oeil_voile: { id: "nyx_oeil_voile", name: "Nyx l’Œil voilé", defaultPrice: 1500 },
  kael_brisefer: { id: "kael_brisefer", name: "Kael Brisefer", defaultPrice: 1600 },
  xalgor_devoreur: { id: "xalgor_devoreur", name: "Xal’gor le Dévoreur", defaultPrice: 1800 },
  ax7_oracle: { id: "ax7_oracle", name: "AX-7, Oracle mécanique", defaultPrice: 1700 }
});

function normalizeUnlockedHeroes(progress) {
  const source = Array.isArray(progress?.unlockedHeroes) ? progress.unlockedHeroes : [];
  const unlocked = [...new Set([DEFAULT_HERO_ID, ...source.filter(heroId => HERO_CATALOG[heroId])])];
  if (progress && typeof progress === "object") progress.unlockedHeroes = unlocked;
  return unlocked;
}
...
function normalizeRewardBundle(value, { maxItems = 8 } = {}) {
  ...
  for (const raw of source) {
    ...
    if (!amount || !["coins", "pack", "card", "hero"].includes(type)) continue;
    ...
    if (type === "hero") {
      const heroId = cleanText(raw?.heroId, 100);
      if (HERO_CATALOG[heroId]) result.push({ type, heroId, amount: 1 });
    }
...
function applyRewards(progress, rewards) {
  ...
  for (const reward of normalizeRewardBundle(rewards, { maxItems: 100 })) {
    ...
    if (reward.type === "hero" && HERO_CATALOG[reward.heroId] && !next.unlockedHeroes.includes(reward.heroId)) next.unlockedHeroes.push(reward.heroId);
  }
  return next;
}
...
function rewardLabel(rewards) {
  return normalizeRewardBundle(rewards).map(reward => {
    ...
    if (reward.type === "hero") return `Héros ${HERO_CATALOG[reward.heroId]?.name || reward.heroId}`;
...
async function initializeExtensions(pool) {
  await pool.query(`
    ...
    CREATE TABLE IF NOT EXISTS pack_shop_settings (
    ...
    CREATE TABLE IF NOT EXISTS hero_shop_settings (
      hero_id VARCHAR(100) PRIMARY KEY,
      enabled BOOLEAN NOT NULL DEFAULT TRUE,
      price INTEGER NOT NULL,
      updated_by_admin_id BIGINT REFERENCES admins(id) ON DELETE SET NULL,
      updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
    ...
    CREATE TABLE IF NOT EXISTS card_shop_listings (
      ...
      created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
      updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
      CONSTRAINT card_shop_listings_dates CHECK (ends_at > starts_at)
    );
    CREATE INDEX IF NOT EXISTS card_shop_listings_active_idx ON card_shop_listings(enabled, starts_at, ends_at);

    CREATE TABLE IF NOT EXISTS card_shop_purchases (
      listing_id BIGINT NOT NULL REFERENCES card_shop_listings(id) ON DELETE CASCADE,
      user_id BIGINT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
      quantity INTEGER NOT NULL DEFAULT 0,
      updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  ...
  for (const pack of Object.values(PACK_CATALOG)) {
    await pool.query(
      `INSERT INTO pack_shop_settings (pack_id, enabled, price)
       VALUES ($1, TRUE, $2)
       ON CONFLICT (pack_id) DO NOTHING`,
      [pack.id, pack.defaultPrice]
    );
  }
  for (const hero of Object.values(HERO_CATALOG)) {
    await pool.query(
      `INSERT INTO hero_shop_settings (hero_id, enabled, price)
       VALUES ($1, $2, $3)
       ON CONFLICT (hero_id) DO NOTHING`,
      [hero.id, hero.id !== DEFAULT_HERO_ID, hero.defaultPrice]
    );
  }
...
function serializePackSetting(row, normalizeId, toMillis) {
  const base = PACK_CATALOG[row.pack_id] || { id: row.pack_id, name: row.pack_id, defaultPrice: Number(row.price || 0) };
  return {
    packId: row.pack_id,
    name: base.name,
    enabled: Boolean(row.enabled),
    price: Number(row.price ?? base.defaultPrice ?? 0),
    defaultPrice: Number(base.defaultPrice ?? 0),
    updatedAt: toMillis(row.updated_at)
  };
}

function serializeHeroSetting(row, normalizeId, toMillis) {
  const base = HERO_CATALOG[row.hero_id] || { id: row.hero_id, name: row.hero_id, defaultPrice: Number(row.price || 0) };
  return {
    heroId: row.hero_id,
    name: base.name,
    enabled: Boolean(row.enabled),
    price: Number(row.price ?? base.defaultPrice ?? 0),
    defaultPrice: Number(base.defaultPrice ?? 0),
    defaultUnlocked: row.hero_id === DEFAULT_HERO_ID,
    updatedAt: toMillis(row.updated_at)
  };
}

function serializeCardListing(row, normalizeId, toMillis) {
  return {
    id: normalizeId(row.id),
    cardId: row.card_id,
    price: Number(row.price || 0),
...
function installExtensionRoutes(deps) {
  ...
  app.get("/api/game/config", async (req, res, next) => {
    try {
      ...
      const [cards, events, quests, offers, packSettings, heroSettings, cardListings] = await Promise.all([
        ...
        pool.query(`SELECT * FROM pack_shop_settings ORDER BY pack_id`),
        pool.query(`SELECT * FROM hero_shop_settings ORDER BY hero_id`),
        pool.query(`SELECT * FROM card_shop_listings WHERE enabled = TRUE AND starts_at <= NOW() AND ends_at > NOW() ORDER BY starts_at, id`)
      ]);
      res.json({
        serverTime: Date.now(),
        ...
        packShopSettings: packSettings.rows.map(row => serializePackSetting(row, normalizeId, toMillis)),
        heroShopSettings: heroSettings.rows.map(row => serializeHeroSetting(row, normalizeId, toMillis)),
        cardShopListings: cardListings.rows.map(row => serializeCardListing(row, normalizeId, toMillis))
      });
  ...
  app.post("/api/shop/packs/:packId/purchase", authMiddleware, async (req, res, next) => {
    const client = await pool.connect();
    try {
      const packId = cleanText(req.params.packId, 100);
      const catalogPack = PACK_CATALOG[packId];
      if (!catalogPack) return res.status(400).json({ error: "Paquet invalide." });
      await client.query("BEGIN");
      const setting = await client.query(`SELECT * FROM pack_shop_settings WHERE pack_id = $1 FOR UPDATE`, [packId]);
      const row = setting.rows[0] || { enabled: true, price: catalogPack.defaultPrice };
      if (!row.enabled) { await client.query("ROLLBACK"); return res.status(409).json({ error: "L’achat de ce paquet est temporairement désactivé." }); }
      const price = Math.max(0, Number(row.price ?? catalogPack.defaultPrice));
      const updated = await mutateProgressInTransaction(client, req.userId, progress => {
        ...
        progress.packs[packId] = Math.max(0, Number(progress.packs[packId] || 0)) + 1;
        return progress;
      });
      await client.query("COMMIT");
      await recordAudit(pool, { category: "purchase", action: "pack_purchased", userId: req.userId, details: { packId, packName: catalogPack.name, price } });
      await notifyAccountUpdated(req.userId, `Paquet acheté : ${catalogPack.name}.`);
      res.json({ ok: true, packId, price, progress: updated.progress, revision: Number(updated.progress_revision || 0) });
    } catch (error) { try { await client.query("ROLLBACK"); } catch (_) {} next(error); }
    finally { client.release(); }
  });

  app.post("/api/shop/heroes/:heroId/purchase", authMiddleware, async (req, res, next) => {
    const client = await pool.connect();
    try {
      const heroId = cleanText(req.params.heroId, 100);
      const catalogHero = HERO_CATALOG[heroId];
      if (!catalogHero || heroId === DEFAULT_HERO_ID) return res.status(400).json({ error: "Héros invalide ou déjà fourni par défaut." });
      await client.query("BEGIN");
      const setting = await client.query(`SELECT * FROM hero_shop_settings WHERE hero_id = $1 FOR UPDATE`, [heroId]);
      const row = setting.rows[0] || { enabled: true, price: catalogHero.defaultPrice };
      if (!row.enabled) { await client.query("ROLLBACK"); return res.status(409).json({ error: "Ce héros n’est pas disponible actuellement." }); }
      const price = Math.max(0, Number(row.price ?? catalogHero.defaultPrice));
      const updated = await mutateProgressInTransaction(client, req.userId, progress => {
        const unlocked = normalizeUnlockedHeroes(progress);
        if (unlocked.includes(heroId)) throw Object.assign(new Error("Ce héros est déjà débloqué."), { statusCode: 409 });
        const coins = Math.max(0, Number(progress.coins || 0));
        if (coins < price) throw Object.assign(new Error("Vous n’avez pas assez de pièces."), { statusCode: 409 });
        progress.coins = coins - price;
      ...
      await client.query("COMMIT");
      await recordAudit(pool, { category: "purchase", action: "hero_purchased", userId: req.userId, details: { heroId, heroName: catalogHero.name, price } });
      await notifyAccountUpdated(req.userId, `Héros débloqué : ${catalogHero.name}.`);
      res.json({ ok: true, heroId, price, progress: updated.progress, revision: Number(updated.progress_revision || 0) });
    } catch (error) { try { await client.query("ROLLBACK"); } catch (_) {} next(error); }
    finally { client.release(); }
  });

  app.post("/api/shop/cards/:id/purchase", authMiddleware, async (req, res, next) => {
    const client = await pool.connect();
    try {
      const listingId = parsePositiveUserId(req.params.id);
      if (!listingId) return res.status(400).json({ error: "Carte en boutique invalide." });
      await client.query("BEGIN");
      const listing = await client.query(`SELECT * FROM card_shop_listings WHERE id = $1 AND enabled = TRUE AND starts_at <= NOW() AND ends_at > NOW() FOR UPDATE`, [listingId]);
      if (!listing.rowCount) { await client.query("ROLLBACK"); return res.status(404).json({ error: "Cette carte n’est plus disponible en boutique." }); }
      const row = listing.rows[0];
      const disabled = await client.query(`SELECT disabled, disabled_until FROM card_overrides WHERE card_id = $1`, [row.card_id]);
      if (disabled.rows[0]?.disabled && (!disabled.rows[0].disabled_until || new Date(disabled.rows[0].disabled_until).getTime() > Date.now())) {
        await client.query("ROLLBACK");
        return res.status(409).json({ error: "Cette carte est temporairement désactivée." });
      }
      const previous = await client.query(`SELECT quantity FROM card_shop_purchases WHERE listing_id = $1 AND user_id = $2 FOR UPDATE`, [listingId, req.userId]);
      const bought = Number(previous.rows[0]?.quantity || 0);
      if (row.purchase_limit > 0 && bought >= row.purchase_limit) {
        await client.query("ROLLBACK");
        return res.status(409).json({ error: "Limite d’achat atteinte pour cette carte." });
      }
      ...
      const updated = await mutateProgressInTransaction(client, req.userId, progress => {
        ...
        progress.collection[row.card_id] = Math.max(0, Number(progress.collection[row.card_id] || 0)) + quantity;
        return progress;
      });
      await client.query(
        `INSERT INTO card_shop_purchases (listing_id, user_id, quantity) VALUES ($1, $2, 1)
         ON CONFLICT (listing_id, user_id) DO UPDATE SET quantity = card_shop_purchases.quantity + 1, updated_at = NOW()`,
        [listingId, req.userId]
      );
      await client.query("COMMIT");
      await recordAudit(pool, { category: "purchase", action: "shop_card_purchased", userId: req.userId, details: { listingId, cardId: row.card_id, price, quantity } });
      await notifyAccountUpdated(req.userId, `Carte achetée : ${row.card_id}.`);
      res.json({ ok: true, listingId: String(listingId), cardId: row.card_id, price, quantity, progress: updated.progress, revision: Number(updated.progress_revision || 0) });
    } catch (error) { try { await client.query("ROLLBACK"); } catch (_) {} next(error); }
    finally { client.release(); }
  });
  ...
  app.post("/api/admin/users/:id/heroes", adminAuthMiddleware, async (req, res, next) => {
    const client = await pool.connect();
    try {
      const userId = parsePositiveUserId(req.params.id);
      const heroId = cleanText(req.body?.heroId, 100);
      const unlocked = req.body?.unlocked !== false;
      if (!userId || !HERO_CATALOG[heroId]) return res.status(400).json({ error: "Joueur ou héros invalide." });
      if (!unlocked && heroId === DEFAULT_HERO_ID) return res.status(409).json({ error: "Le héros de départ ne peut pas être verrouillé." });
      await client.query("BEGIN");
      const updated = await mutateProgressInTransaction(client, userId, progress => {
        normalizeUnlockedHeroes(progress);
        const set = new Set(progress.unlockedHeroes);
        if (unlocked) set.add(heroId); else set.delete(heroId);
        set.add(DEFAULT_HERO_ID);
        progress.unlockedHeroes = [...set];
        if (!progress.unlockedHeroes.includes(progress.selectedHeroId)) progress.selectedHeroId = DEFAULT_HERO_ID;
        if (Array.isArray(progress.decks)) {
          progress.decks = progress.decks.map(deck => ({ ...deck, heroId: progress.unlockedHeroes.includes(deck?.heroId) ? deck.heroId : DEFAULT_HERO_ID }));
        }
        return progress;
      });
      if (!updated) { await client.query("ROLLBACK"); return res.status(404).json({ error: "Joueur introuvable." }); }
      await client.query("COMMIT");
      await recordAudit(pool, { category: "admin", action: unlocked ? "hero_unlocked" : "hero_locked", userId, adminId: req.adminId, details: { heroId, heroName: HERO_CATALOG[heroId].name } });
      await notifyAccountUpdated(userId, unlocked ? `Le héros ${HERO_CATALOG[heroId].name} a été débloqué par un administrateur.` : `Le héros ${HERO_CATALOG[heroId].name} a été verrouillé par un administrateur.`);
      res.json({ ok: true, heroId, unlocked, progress: updated.progress, revision: Number(updated.progress_revision || 0) });
    } catch (error) { try { await client.query("ROLLBACK"); } catch (_) {} next(error); }
    finally { client.release(); }
  });

  // -------------------- Boutique --------------------
  app.get("/api/admin/shop-packs", adminAuthMiddleware, async (_req, res, next) => {
    try {
      const result = await pool.query("SELECT * FROM pack_shop_settings ORDER BY pack_id");
      res.json({ packs: result.rows.map(row => serializePackSetting(row, normalizeId, toMillis)) });
    } catch (error) { next(error); }
  });
  app.put("/api/admin/shop-packs/:packId", adminAuthMiddleware, async (req, res, next) => {
    try {
      const packId = cleanText(req.params.packId, 100);
      const pack = PACK_CATALOG[packId];
      const price = positiveInt(req.body?.price, 0, 100000000);
      const enabled = req.body?.enabled !== false;
      if (!pack || price === null) return res.status(400).json({ error: "Paquet ou prix invalide." });
      const result = await pool.query(
        `INSERT INTO pack_shop_settings (pack_id, enabled, price, updated_by_admin_id, updated_at)
         VALUES ($1, $2, $3, $4, NOW())
         ON CONFLICT (pack_id) DO UPDATE SET enabled = EXCLUDED.enabled, price = EXCLUDED.price,
           updated_by_admin_id = EXCLUDED.updated_by_admin_id, updated_at = NOW()
         RETURNING *`,
        [packId, enabled, price, req.adminId]
      );
      await recordAudit(pool, { category: "admin", action: "pack_shop_updated", adminId: req.adminId, details: { packId, enabled, price } });
      broadcastToAllPlayers({ type: "game_config_changed", section: "shop" });
      res.json({ pack: serializePackSetting(result.rows[0], normalizeId, toMillis) });
    } catch (error) { next(error); }
  });

  app.get("/api/admin/shop-heroes", adminAuthMiddleware, async (_req, res, next) => {
    try {
      const result = await pool.query("SELECT * FROM hero_shop_settings ORDER BY hero_id");
      res.json({ heroes: result.rows.map(row => serializeHeroSetting(row, normalizeId, toMillis)) });
    } catch (error) { next(error); }
  });
  app.put("/api/admin/shop-heroes/:heroId", adminAuthMiddleware, async (req, res, next) => {
    try {
      const heroId = cleanText(req.params.heroId, 100);
      const hero = HERO_CATALOG[heroId];
      const price = positiveInt(req.body?.price, 0, 100000000);
      const enabled = heroId === DEFAULT_HERO_ID ? false : req.body?.enabled !== false;
      if (!hero || price === null) return res.status(400).json({ error: "Héros ou prix invalide." });
      const result = await pool.query(
        `INSERT INTO hero_shop_settings (hero_id, enabled, price, updated_by_admin_id, updated_at)
         VALUES ($1, $2, $3, $4, NOW())
         ON CONFLICT (hero_id) DO UPDATE SET enabled = EXCLUDED.enabled, price = EXCLUDED.price,
           updated_by_admin_id = EXCLUDED.updated_by_admin_id, updated_at = NOW()
         RETURNING *`,
        [heroId, enabled, price, req.adminId]
      );
      await recordAudit(pool, { category: "admin", action: "hero_shop_updated", adminId: req.adminId, details: { heroId, enabled, price } });
      broadcastToAllPlayers({ type: "game_config_changed", section: "shop" });
      res.json({ hero: serializeHeroSetting(result.rows[0], normalizeId, toMillis) });
    } catch (error) { next(error); }
  });

  app.get("/api/admin/card-shop", adminAuthMiddleware, async (_req, res, next) => {
    try {
      const result = await pool.query("SELECT * FROM card_shop_listings ORDER BY created_at DESC, id DESC");
      res.json({ listings: result.rows.map(row => serializeCardListing(row, normalizeId, toMillis)) });
    } catch (error) { next(error); }
  });
  async function saveCardListing(req, res, next, listingId = null) {
    try {
      const cardId = cleanText(req.body?.cardId, 100);
      const price = positiveInt(req.body?.price, 0, 100000000);
      const hasOriginalPrice = req.body?.originalPrice !== "" && req.body?.originalPrice != null;
      const originalPrice = hasOriginalPrice ? positiveInt(req.body.originalPrice, 0, 100000000) : null;
      const quantity = positiveInt(req.body?.quantity ?? 1, 1, 1000);
      const purchaseLimit = positiveInt(req.body?.purchaseLimit ?? 0, 0, 1000000);
      const startsAt = safeDate(req.body?.startsAt, { required: true });
      const endsAt = safeDate(req.body?.endsAt, { required: true });
      const enabled = req.body?.enabled !== false;
      if (!/^[a-zA-Z0-9_-]{1,100}$/.test(cardId) || price === null || (hasOriginalPrice && originalPrice === null) || quantity === null || purchaseLimit === null || endsAt <= startsAt) {
        return res.status(400).json({ error: "Données de carte en boutique invalides." });
      }
      let result;
      if (listingId) {
        result = await pool.query(
          `UPDATE card_shop_listings SET card_id=$1, price=$2, original_price=$3, quantity=$4, purchase_limit=$5,
           enabled=$6, starts_at=$7, ends_at=$8, updated_at=NOW() WHERE id=$9 RETURNING *`,
          [cardId, price, originalPrice, quantity, purchaseLimit, enabled, startsAt, endsAt, listingId]
        );
      } else {
        result = await pool.query(
          `INSERT INTO card_shop_listings (card_id, price, original_price, quantity, purchase_limit, enabled, starts_at, ends_at, created_by_admin_id)
           VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING *`,
          [cardId, price, originalPrice, quantity, purchaseLimit, enabled, startsAt, endsAt, req.adminId]
        );
      }
      if (!result.rowCount) return res.status(404).json({ error: "Carte en boutique introuvable." });
      await recordAudit(pool, { category: "admin", action: listingId ? "card_shop_updated" : "card_shop_created", adminId: req.adminId, details: { listingId: listingId || normalizeId(result.rows[0].id), cardId, price, quantity, startsAt, endsAt } });
      broadcastToAllPlayers({ type: "game_config_changed", section: "shop" });
      res.status(listingId ? 200 : 201).json({ listing: serializeCardListing(result.rows[0], normalizeId, toMillis) });
    } catch (error) { next(error); }
  }
  app.post("/api/admin/card-shop", adminAuthMiddleware, (req,res,next) => saveCardListing(req,res,next,null));
  app.put("/api/admin/card-shop/:id", adminAuthMiddleware, (req,res,next) => {
    const id = parsePositiveUserId(req.params.id);
    if (!id) return res.status(400).json({ error: "Carte en boutique invalide." });
    return saveCardListing(req,res,next,id);
  });
  app.delete("/api/admin/card-shop/:id", adminAuthMiddleware, async (req,res,next) => {
    try {
      const id = parsePositiveUserId(req.params.id);
      if (!id) return res.status(400).json({ error: "Carte en boutique invalide." });
      const result = await pool.query("DELETE FROM card_shop_listings WHERE id=$1 RETURNING card_id", [id]);
      if (!result.rowCount) return res.status(404).json({ error: "Carte en boutique introuvable." });
      await recordAudit(pool, { category: "admin", action: "card_shop_deleted", adminId: req.adminId, details: { listingId: id, cardId: result.rows[0].card_id } });
      broadcastToAllPlayers({ type: "game_config_changed", section: "shop" });
      res.json({ ok: true });
    } catch (error) { next(error); }
  });
