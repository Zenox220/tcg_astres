...
const ALLOWED_BATTLE_MODES = new Set(["standard", "bras_de_fer", "sans_repis", "furie_decks", "tavernier"]);
const normalizeBattleMode = value => ALLOWED_BATTLE_MODES.has(String(value || "").trim()) ? String(value).trim() : "standard";
const matchQueue = []; // [{ userId, username, deck, heroId, battleMode }]
const rooms = new Map(); // roomId -> { host, guest, battleMode }
...
async function handleWsMessage(userId, username, _ws, msg) {
  ...
  if (msg.type === "queue_join") {
    ...
    const entry = { userId, username, deck: msg.deck || [], heroId: msg.heroId || null, battleMode };
    const opponentIndex = matchQueue.findIndex(candidate => candidate.battleMode === battleMode);
    if (opponentIndex < 0) {
      matchQueue.push(entry);
      sendTo(userId, { type: "queue_waiting", battleMode });
      return;
    }

    const hostEntry = matchQueue.splice(opponentIndex, 1)[0];
    const guestEntry = entry;
    const roomId = crypto.randomUUID();
    rooms.set(roomId, { host: hostEntry.userId, guest: guestEntry.userId, battleMode });
