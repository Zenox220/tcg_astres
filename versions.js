...
(() => {
  ...
  function getCardSoundCandidates(card) {
    ...
    for (const extension of extensions) candidates.push(`audio/cards/${slug}.${extension}`);
    ...
    if (id) for (const extension of extensions) candidates.push(`audio/cards/${id}.${extension}`);
    if (id) for (const extension of extensions) candidates.push(`audio/cartes/${id}.${extension}`);
    if (exactName) for (const extension of extensions) candidates.push(`audio/cards/${exactName}.${extension}`);
...
(() => {
  ...
  async function apiFetch(path, { method = "GET", body } = {}) {
    ...
    if (!res.ok) {
      ...
      error.data = data;
      error.ban = data.ban || null;
      error.progress = data.progress;
      error.revision = Number(data.revision || 0);
      throw error;
    }
    return data;
  }
  ...
  function clearStoredProgressForCurrentProfile() {
    const key = getProgressStorageKey();
    try { window.localStorage?.removeItem?.(key); } catch (_) { storageSet(key, ""); }
  }

  function applyAuthoritativeProgress(remoteProgress, revision, message = null) {
    accountProgressRevision = Math.max(0, Number(revision || 0));
    if (remoteProgress && typeof remoteProgress === "object" && !Array.isArray(remoteProgress)) {
      storeProgressForCurrentProfile(remoteProgress);
    } else {
      clearStoredProgressForCurrentProfile();
    }
  ...
  function queueAccountProgressUpload(snapshot, { silent = false } = {}) {
    ...
    progressSaveQueue = progressSaveQueue
      .catch(() => null)
      .then(() => apiFetch("/api/progress", { method: "PUT", body: { progress: safeSnapshot, revision: accountProgressRevision } }))
      .then(result => {
        if (result?.revision !== undefined) accountProgressRevision = Number(result.revision || 0);
        return result;
      })
      .catch(err => {
        if (err.code === "PROGRESS_OUTDATED") {
          applyAuthoritativeProgress(err.progress, err.revision, "Votre progression a été actualisée après une modification administrateur.");
          return null;
        }
        if (err.code === "ACCOUNT_BANNED") {
          showAccountBan(err.ban);
          return null;
        }
  ...
  window.astreaFlushAccountProgress = flushAccountProgressSave;
  ...
  async function activateAccountProgress(fallbackSnapshot, { isRegistration = false } = {}) {
    ...
    accountProgressRevision = Math.max(0, Number(remote.revision || 0));
    ...
    if (remote.authoritative) {
      if (remote.progress) storeProgressForCurrentProfile(remote.progress);
      else clearStoredProgressForCurrentProfile();
      reloadProgressForCurrentProfile();
      chosen = cloneProgressSnapshot(progress);
    } else {
      chosen = remote.progress || (isRegistration ? fallback : (cachedAccountProgress || fallback));
      if (chosen) {
    ...
    if (chosen) {
      await queueAccountProgressUpload(progress, { silent: Boolean(remote.progress || remote.authoritative) });
    }
  }
  ...
  function formatBanDate(timestamp) {
    if (!timestamp) return "définitivement";
    try {
      return `jusqu’au ${new Date(timestamp).toLocaleString("fr-FR", { dateStyle: "long", timeStyle: "short" })}`;
    } catch (_) { return "temporairement"; }
  }

  function showAccountBan(ban = {}) {
    accountProgressReady = false;
    window.clearTimeout(progressSaveTimer);
    progressSaveTimer = null;
    ...
    setSession(null, null);
    returnToGuestProgress();
    if (dom.accountBanDuration) {
      dom.accountBanDuration.textContent = ban.permanent || !ban.expiresAt
        ? "Bannissement permanent"
        : `Bannissement ${formatBanDate(ban.expiresAt)}`;
    }
    if (dom.accountBanReason) dom.accountBanReason.textContent = ban.reason || "Aucune raison communiquée.";
    dom.accountBanModal?.classList.remove("hidden");
  }

  function closeAccountBanModal() {
    dom.accountBanModal?.classList.add("hidden");
  }
  ...
  function handleWsMessage(msg) {
    switch (msg.type) {
      ...
      case "match_found":
        ...
        if (msg.role === "host") startPvpBattleAsHost(msg.opponent, msg.roomId, msg.battleMode || "standard");
        else startPvpBattleAsGuest(msg.opponent, msg.roomId, msg.battleMode || "standard");
        break;
      case "pvp_state":
        handlePvpState(msg);
        break;
      ...
      case "account_updated":
        applyAuthoritativeProgress(msg.progress, msg.revision, msg.message || "Votre compte a été actualisé par un administrateur.");
        break;
      case "account_session_revoked":
        ...
        setSession(null, null);
        returnToGuestProgress();
        showToast(escapeHtml(msg.message || "Votre session a été fermée par un administrateur."), 7000);
        break;
      case "account_banned":
        showAccountBan(msg.ban || {});
        break;
      case "game_popup_changed":
      case "game_popup_refresh":
        window.astreaPopupManager?.receive(msg);
        break;
      case "account_warning":
      case "account_muted":
      case "account_unmuted":
      case "game_config_changed":
        window.astreaLiveAdmin?.receive?.(msg);
        break;
      default:
        break;
    }
  ...
  function openFriendActionModal(friendUsername) {
    ...
    if (dom.friendReportCategory) dom.friendReportCategory.value = "Comportement toxique";
    dom.friendReportError?.classList.add("hidden");
  ...
  async function sendFriendReport() {
    const reason = String(dom.friendReportText?.value || "").trim();
    const category = String(dom.friendReportCategory?.value || "Autre").trim();
    if (!activeFriendActionTarget) return;
    dom.friendReportError?.classList.add("hidden");
    if (reason.length < 10) {
      if (dom.friendReportError) {
        dom.friendReportError.textContent = "Expliquez le problème en au moins 10 caractères.";
        dom.friendReportError.classList.remove("hidden");
      }
      dom.friendReportText?.focus();
      return;
    }
    if (dom.friendReportSendBtn) dom.friendReportSendBtn.disabled = true;
    try {
      const result = await apiFetch("/api/report", {
        method: "POST",
        body: { username: activeFriendActionTarget, category, reason, sourceUrl: window.location.href }
      });
      const suffix = result.warning ? " Le signalement est enregistré, mais Discord n’a pas pu être contacté." : "";
      showToast(`Signalement envoyé pour <strong>${escapeHtml(activeFriendActionTarget)}</strong>.${suffix}`, 6000);
      closeFriendActionModal();
    } catch (err) {
      if (dom.friendReportError) {
        dom.friendReportError.textContent = err.message;
        dom.friendReportError.classList.remove("hidden");
      }
    } finally {
      if (dom.friendReportSendBtn) dom.friendReportSendBtn.disabled = false;
    }
  }
  ...
  function openMultiplayerFlow() {
    ...
    const battleMode = window.ASTREA_BATTLE_MODES_V44?.normalizeMode?.(window.ASTREA_PENDING_BATTLE_MODE) || "standard";
    const ignoresConstructedDeck = battleMode === "furie_decks" || battleMode === "tavernier";
    if (!ignoresConstructedDeck && !isDeckStructurallyValid(progress.deck, progress.collection)) {
      showToast("Votre deck doit contenir 30 cartes valides avant de jouer en multijoueur.");
      openCollection();
      return;
    }
    const matchmakingText = dom.matchmakingModal?.querySelector("p");
    if (matchmakingText) {
      const label = window.ASTREA_BATTLE_MODES_V44?.getModeLabel?.(battleMode) || "Classique";
      matchmakingText.textContent = `Recherche d’un adversaire — ${label}…`;
    }
    dom.matchmakingModal?.classList.remove("hidden");
    wsSend({
      type: "queue_join",
      deck: ignoresConstructedDeck ? [] : progress.deck,
      heroId: progress.selectedHeroId,
      battleMode
    });
    window.ASTREA_PENDING_BATTLE_MODE = null;
  }
  ...
  function startPvpBattleAsHost(opponent, roomId, requestedBattleMode = "standard") {
    resetHeroDefeatVisuals();
    const battleMode = window.ASTREA_BATTLE_MODES_V44?.normalizeMode?.(requestedBattleMode) || "standard";
    const ignoresConstructedDeck = battleMode === "furie_decks" || battleMode === "tavernier";
    if (!ignoresConstructedDeck && !isDeckStructurallyValid(progress.deck, progress.collection)) {
    ...
    if (battleMode === "tavernier" && window.ASTREA_BATTLE_MODES_V44?.startPvpTavernierHost) {
      window.ASTREA_BATTLE_MODES_V44.startPvpTavernierHost({
        opponent,
        roomId,
        hostUsername: username,
        showGameScreen: showGameScreenForBattle,
        broadcastState: broadcastPvpState
      });
      return;
    }
    ...
    let playerDeckList = progress.deck;
    let opponentDeckList = Array.isArray(opponent.deck) && opponent.deck.length === CONFIG.DECK_SIZE ? opponent.deck : AI_DECK_LIST;
    if (battleMode === "furie_decks") {
      playerDeckList = window.ASTREA_BATTLE_MODES_V44?.buildUniqueRandomDeck?.("pvp_host") || AI_DECK_LIST;
      opponentDeckList = window.ASTREA_BATTLE_MODES_V44?.buildUniqueRandomDeck?.("pvp_guest", [playerDeckList]) || AI_DECK_LIST;
    }

    gameState = {
      status: "playing", mode: "pvp", battleMode, adventureId: null, turnNumber: 1, activePlayerId: firstPlayerId, firstPlayerId,
    ...
    window.ASTREA_BATTLE_MODES_V44?.configureNormalBattleState?.(gameState, battleMode);

    showGameScreenForBattle();
    const modeLabel = window.ASTREA_BATTLE_MODES_V44?.getModeLabel?.(battleMode) || "Classique";
    addLog(firstPlayerId === "player"
      ? `Duel ${modeLabel} contre ${opponent.username}. Vous jouez en premier.`
      : `Duel ${modeLabel} contre ${opponent.username}. ${opponent.username} joue en premier. Vous recevez La pièce.`);
    renderGame();
  }

  function startPvpBattleAsGuest(opponent, roomId, requestedBattleMode = "standard") {
    const battleMode = window.ASTREA_BATTLE_MODES_V44?.normalizeMode?.(requestedBattleMode) || "standard";
    pvpRole = "guest";
    pvpRoomId = roomId;
    pvpOpponentName = opponent.username;
    ...
    showGameScreenForBattle();
    const modeLabel = window.ASTREA_BATTLE_MODES_V44?.getModeLabel?.(battleMode) || "Classique";
    showPvpWaitingOverlay(`Connexion au duel ${modeLabel} contre ${opponent.username}…`);
  }
  ...
  function handlePvpState(msg) {
    ...
    hidePvpWaitingOverlay();
    window.ASTREA_BATTLE_MODES_V44?.onPvpStateReceived?.(gameState);
    renderGame();
  }
  ...
  function applyGuestAction(action) {
    ...
    if (window.ASTREA_BATTLE_MODES_V44?.applyPvpGuestAction?.(action, { broadcastState: broadcastPvpState })) return;
    ...
    } else if (action.kind === "heroPowerV43") {
      window.ASTREA_HERO_POWERS_V43?.applySeatPower?.("ai", action.payload || {});
  ...
  usePlayerHeroPower = function () {
    ...
    if (window.ASTREA_HERO_POWERS_V43?.supportsPowerType?.(powerType)) {
      if (pvpRole === "guest") {
        return window.ASTREA_HERO_POWERS_V43.beginGuestPower({
          sendAction: sendPvpAction,
          flipTargetSeat
        });
      }
      return window.ASTREA_HERO_POWERS_V43.useLocalPower("player");
    }
    if (pvpRole === "guest") {
      if (!canUseHeroPower("player")) return;
  ...
  executePendingAction = async function (target) {
    const pending = gameState?.pendingAction;
    if (pending?.type === "heroPowerV43") {
      if (pvpRole === "guest") {
        return window.ASTREA_HERO_POWERS_V43?.completeGuestTarget?.({
          target,
          sendAction: sendPvpAction,
          flipTargetSeat
        });
      }
      return window.ASTREA_HERO_POWERS_V43?.completeLocalTarget?.("player", target);
    }
  ...
  window.ASTREA_PVP_BRIDGE = {
    getRole: () => pvpRole,
    getRoomId: () => pvpRoomId,
    getOpponentName: () => pvpOpponentName,
    getUsername: () => username,
    openMultiplayerFlow,
    sendAction: sendPvpAction,
    broadcastState: broadcastPvpState,
    showGameScreen: showGameScreenForBattle,
    showWaiting: showPvpWaitingOverlay,
    hideWaiting: hidePvpWaitingOverlay
  };
  ...
  function cacheV34Dom() {
    const ids = [
      ...
      "friend-report-panel", "friend-report-category", "friend-report-text", "friend-report-error", "friend-report-send-btn",
      "account-ban-modal", "account-ban-duration", "account-ban-reason", "account-ban-understand-btn",
  ...
  function bindV34Events() {
    ...
    dom.accountBanUnderstandBtn?.addEventListener("click", closeAccountBanModal);
    ...
    dom.friendActionReportBtn?.addEventListener("click", () => {
      const willOpen = dom.friendReportPanel?.classList.contains("hidden");
      dom.friendReportPanel?.classList.toggle("hidden");
      if (willOpen) window.setTimeout(() => dom.friendReportText?.focus(), 30);
    });
...
*/
(() => {
  const ENCHANT_DUPLICATE_COST = 10;
...
/* ==========================================================================
   DÉBUT DE LA VERSION V38
   ========================================================================== */
/* Chroniques d'Astréa V38 — Pop-up publiées depuis le panel administrateur
   -------------------------------------------------------------------------
   - Récupère les annonces actives depuis PostgreSQL, y compris pour les
     joueurs invités qui ne sont pas connectés à un compte.
   - Affiche plusieurs images sous forme de diaporama, une description, un
     footer et jusqu'à quatre boutons sécurisés.
   - Une même version de pop-up n'est affichée qu'une fois par navigateur.
   - Les joueurs connectés reçoivent immédiatement les nouvelles annonces via
     le WebSocket existant ; un contrôle périodique couvre aussi les invités.
*/
(() => {
  const RAW_SERVER_URL = String(window.ASTREA_SERVER_URL || "").trim().replace(/\/+$/, "");
  const API_BASE_URL = RAW_SERVER_URL && !RAW_SERVER_URL.includes("REMPLACEZ-MOI") ? RAW_SERVER_URL : "";
  const SEEN_PREFIX = "astrea_game_popup_seen_v1_";
  ...
  function popupAuthToken() {
    try { return String(window.localStorage?.getItem?.("astrea_auth_token") || ""); }
    catch (_) { return ""; }
  }
  ...
  function normalizePopup(raw) {
    if (!raw || raw.id === undefined || !raw.title) return null;
    const images = (Array.isArray(raw.images) ? raw.images : []).map(safeHttpUrl).filter(Boolean).slice(0, 8);
    const buttons = (Array.isArray(raw.buttons) ? raw.buttons : []).map(button => ({
      label: String(button?.label || "").trim().slice(0, 60),
      url: safeHttpUrl(button?.url),
      style: ["primary", "secondary", "danger"].includes(button?.style) ? button.style : "primary"
    ...
    return {
      ...
      footer: String(raw.footer || "").trim().slice(0, 250),
      images,
      buttons,
      reward: (Array.isArray(raw.reward) ? raw.reward : (raw.reward && typeof raw.reward === "object" ? [raw.reward] : [])).filter(item => ["coins", "pack", "card", "hero"].includes(item?.type)),
  ...
  function isPopupActive(popup) {
    const now = Date.now();
    return Boolean(popup?.enabled && popup.startsAt <= now && popup.expiresAt > now);
  }

  function seenKey(popup) {
    return `${SEEN_PREFIX}${popup.id}_${popup.updatedAt}`;
  }

  function hasSeen(popup) {
    try { return window.localStorage?.getItem?.(seenKey(popup)) === "1"; }
    catch (_) { return false; }
  }

  function markSeen(popup) {
    if (!popup) return;
    try { window.localStorage?.setItem?.(seenKey(popup), "1"); } catch (_) { /* stockage indisponible */ }
  }

  function cachePopupDom() {
    const ids = [
      "game-popup-modal", "game-popup-close-btn", "game-popup-image-wrap", "game-popup-image",
      "game-popup-prev-btn", "game-popup-next-btn", "game-popup-image-count", "game-popup-title",
      "game-popup-description", "game-popup-buttons", "game-popup-footer"
    ];
    ids.forEach(id => { popupDom[id.replace(/-([a-z])/g, (_, letter) => letter.toUpperCase())] = document.getElementById(id); });
  }
  ...
  function renderPopupImage() {
    const images = currentPopup?.images || [];
    const hasImages = images.length > 0;
    popupDom.gamePopupImageWrap?.classList.toggle("hidden", !hasImages);
    if (!hasImages) return;
    currentImageIndex = Math.max(0, Math.min(currentImageIndex, images.length - 1));
    if (popupDom.gamePopupImage) {
      popupDom.gamePopupImage.onerror = () => {
        popupDom.gamePopupImage.alt = "Image de l’annonce indisponible";
      };
      popupDom.gamePopupImage.src = images[currentImageIndex];
      popupDom.gamePopupImage.alt = `Illustration ${currentImageIndex + 1} de l’annonce ${currentPopup.title}`;
    }
    const multiple = images.length > 1;
    popupDom.gamePopupPrevBtn?.classList.toggle("hidden", !multiple);
    popupDom.gamePopupNextBtn?.classList.toggle("hidden", !multiple);
    popupDom.gamePopupImageCount?.classList.toggle("hidden", !multiple);
    if (popupDom.gamePopupImageCount) popupDom.gamePopupImageCount.textContent = `${currentImageIndex + 1} / ${images.length}`;
  }

  function renderCurrentPopup() {
    if (!currentPopup) return;
    currentImageIndex = 0;
    popupDom.gamePopupTitle.textContent = currentPopup.title;
    popupDom.gamePopupDescription.textContent = currentPopup.description;
    popupDom.gamePopupDescription.classList.toggle("hidden", !currentPopup.description);
    popupDom.gamePopupFooter.textContent = currentPopup.footer;
    popupDom.gamePopupFooter.classList.toggle("hidden", !currentPopup.footer);
    const linkButtons = currentPopup.buttons.map(button =>
      `<a class="game-popup-button ${escapePopupHtml(button.style)}" href="${escapePopupHtml(button.url)}" target="_blank" rel="noopener noreferrer">${escapePopupHtml(button.label)}</a>`
    ).join("");
    const hasReward = currentPopup.reward.length > 0;
    const rewardButton = hasReward ? `<button class="game-popup-button game-popup-reward-button" data-popup-claim type="button" ${currentPopup.claimed ? "disabled" : ""}>${currentPopup.claimed ? "Récompense récupérée" : "Récupérer"}</button>` : "";
    popupDom.gamePopupButtons.innerHTML = linkButtons + rewardButton;
    popupDom.gamePopupButtons.classList.toggle("hidden", currentPopup.buttons.length === 0 && !hasReward);
    popupDom.gamePopupButtons.querySelector("[data-popup-claim]")?.addEventListener("click", claimCurrentReward);
    renderPopupImage();
    popupDom.gamePopupModal.classList.remove("hidden");
    window.setTimeout(() => popupDom.gamePopupCloseBtn?.focus(), 30);
  }

  async function claimCurrentReward() {
    if (!currentPopup || currentPopup.claimed || !currentPopup.reward.length) return;
    const token = popupAuthToken();
    if (!token) {
      window.astreaLiveAdmin?.toast?.("Connectez-vous à votre compte pour récupérer cette récompense.", true);
      return;
    }
    const button = popupDom.gamePopupButtons?.querySelector("[data-popup-claim]");
    if (button) { button.disabled = true; button.textContent = "Récupération…"; }
    try {
      const response = await fetch(`${API_BASE_URL}/api/popups/${encodeURIComponent(currentPopup.id)}/claim`, {
        method: "POST",
        headers: { Accept: "application/json", Authorization: `Bearer ${token}` }
      });
      const data = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(data.error || "Récompense indisponible.");
      currentPopup.claimed = true;
      window.astreaApplyServerProgress?.(data.progress);
      window.astreaLiveAdmin?.toast?.(data.rewardLabel || "Récompense ajoutée à votre compte.");
      renderCurrentPopup();
    } catch (error) {
      if (button) { button.disabled = false; button.textContent = "Récupérer"; }
      window.astreaLiveAdmin?.toast?.(error.message || "Impossible de récupérer la récompense.", true);
    }
  }

  function scheduleNextPopup() {
    window.clearTimeout(showRetryTimer);
    if (currentPopup || !queue.length) return;
    if (isLoadingScreenVisible()) {
      showRetryTimer = window.setTimeout(scheduleNextPopup, 500);
      return;
    }
    const next = queue.shift();
    if (!isPopupActive(next) || hasSeen(next)) return scheduleNextPopup();
    currentPopup = next;
    renderCurrentPopup();
  }

  function enqueuePopup(rawPopup, { forceRefresh = false } = {}) {
    const popup = normalizePopup(rawPopup);
    if (!popup || !isPopupActive(popup) || hasSeen(popup)) return;
    if (currentPopup?.id === popup.id) {
      if (forceRefresh || currentPopup.updatedAt !== popup.updatedAt) {
        currentPopup = popup;
        renderCurrentPopup();
      }
      return;
    }
    queue = queue.filter(item => item.id !== popup.id);
    queue.push(popup);
    queue.sort((a, b) => a.startsAt - b.startsAt || Number(a.id) - Number(b.id));
    scheduleNextPopup();
  }

  function closeCurrentPopup({ remember = true } = {}) {
    if (!currentPopup) {
      popupDom.gamePopupModal?.classList.add("hidden");
      return;
    }
    if (remember) markSeen(currentPopup);
    currentPopup = null;
    popupDom.gamePopupModal?.classList.add("hidden");
    window.setTimeout(scheduleNextPopup, 120);
  }

  function removePopup(popupId) {
    const id = String(popupId || "");
    queue = queue.filter(item => item.id !== id);
    if (currentPopup?.id === id) closeCurrentPopup({ remember: false });
  }

  async function fetchActivePopups() {
    if (!API_BASE_URL && window.location.hostname.endsWith("github.io")) return;
    try {
      const token = popupAuthToken();
      const headers = { Accept: "application/json" };
      if (token) headers.Authorization = `Bearer ${token}`;
      const response = await fetch(`${API_BASE_URL}/api/popups/active`, { headers, cache: "no-store" });
      if (!response.ok) return;
      const data = await response.json();
      const active = (Array.isArray(data.popups) ? data.popups : []).map(normalizePopup).filter(Boolean);
      const activeIds = new Set(active.map(popup => popup.id));
      queue = queue.filter(popup => activeIds.has(popup.id) && isPopupActive(popup));
      if (currentPopup && !activeIds.has(currentPopup.id)) closeCurrentPopup({ remember: false });
      active.forEach(popup => enqueuePopup(popup));
    } catch (_) { /* le jeu reste utilisable si Render est momentanément indisponible */ }
  }

  function receiveSocketMessage(message) {
    if (!message) return;
    if (message.type === "game_popup_refresh") { fetchActivePopups(); return; }
    if (message.type !== "game_popup_changed") return;
    if (message.action === "delete") {
      removePopup(message.popupId);
      return;
    }
    const popup = normalizePopup(message.popup);
    if (!popup || !isPopupActive(popup)) {
      if (popup?.id) removePopup(popup.id);
      return;
    }
    enqueuePopup(popup, { forceRefresh: true });
  }

  function bindPopupEvents() {
    popupDom.gamePopupCloseBtn?.addEventListener("click", () => closeCurrentPopup());
    popupDom.gamePopupPrevBtn?.addEventListener("click", () => {
      if (!currentPopup?.images?.length) return;
      currentImageIndex = (currentImageIndex - 1 + currentPopup.images.length) % currentPopup.images.length;
      renderPopupImage();
    });
    popupDom.gamePopupNextBtn?.addEventListener("click", () => {
      if (!currentPopup?.images?.length) return;
      currentImageIndex = (currentImageIndex + 1) % currentPopup.images.length;
      renderPopupImage();
    });
    document.addEventListener("keydown", event => {
      if (popupDom.gamePopupModal?.classList.contains("hidden")) return;
      if (event.key === "Escape") closeCurrentPopup();
      if (event.key === "ArrowLeft") popupDom.gamePopupPrevBtn?.click();
      if (event.key === "ArrowRight") popupDom.gamePopupNextBtn?.click();
    });
  }

  window.astreaPopupManager = { receive: receiveSocketMessage, refresh: fetchActivePopups };

  document.addEventListener("DOMContentLoaded", () => {
    cachePopupDom();
    bindPopupEvents();
    fetchActivePopups();
    pollTimer = window.setInterval(fetchActivePopups, POLL_INTERVAL_MS);
    window.addEventListener("beforeunload", () => window.clearInterval(pollTimer), { once: true });
  });
})();
/* ==========================================================================
   FIN DE LA VERSION V38
   ========================================================================== */
